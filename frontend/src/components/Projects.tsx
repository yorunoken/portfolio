            ...
            <div className="space-y-12">
                {projects.map((project) => (
                    <div key={project.title} className="group md:grid md:grid-cols-4 md:gap-6 flex flex-col gap-2 border-l border-transparent pl-4 md:pl-0 md:border-l-0 transition-colors">
                        <div className="text-xs text-zinc-500 font-mono pt-1 w-full shrink-0 tracking-wider uppercase">
                            {project.dates}
                        </div>
                        ...
                        <div className="md:col-span-3 space-y-3">
                            <div className="flex flex-wrap items-baseline gap-x-3 gap-y-1">
                                <h3 className="text-base font-semibold text-white tracking-tight">
                                    {project.link ? (
                                        <a href={project.link} target="_blank" rel="noopener noreferrer" className="hover:text-blue-300 hover:underline underline-offset-4 decoration-blue-500/40 transition-colors">
                                            {project.title}
                                        </a>
                                    ) : (
                                        project.title
                            ...
                            <div className="flex items-center flex-wrap gap-3 text-xs font-mono">
                                ...
                                {project.github && (
                                    <a
                                        ...
                                        className="inline-flex items-center gap-1 text-zinc-500 hover:text-zinc-200 transition-colors py-0.5 px-2 rounded-md bg-white/2 border border-white/5"
                                    >
                                        <GitHubIcon />
                            ...
                            <div className="flex flex-wrap gap-1.5 pt-1.5">
                                {project.tags.map((tag, tagIndex) => (
                                    <span key={tagIndex} className="px-2 py-0.5 bg-white/2 border border-white/5 text-[11px] font-mono rounded text-zinc-400 transition-all duration-300 hover:text-zinc-200 hover:border-white/10">
                                        {tag}
                                    </span>
                                ))}
                            </div>
